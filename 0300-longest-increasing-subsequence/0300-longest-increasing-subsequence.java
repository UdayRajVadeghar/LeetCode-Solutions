class Solution {
    ...
    //404 MS Memoization - Learn Tabulation Soon
    ...
    public int lengthOfLIS(int[] nums){
        ...
        Integer[][] dp = new Integer[nums.length+1][nums.length+2];

        return recurse(nums , 0 , nums.length , -1 , dp);



    }
    public static int recurse(int[] nums , int index , int length ,int lastPicked , Integer[][] dp){

        if(index == length){
            return 0;
        }

        if(dp[index][lastPicked+1] != null){
        ...
        if(lastPicked == -1 || nums[index] > nums[lastPicked]){
            left = 1 + recurse(nums , index + 1, length , index , dp);
        }
            
        int right = recurse(nums , index + 1 , length , lastPicked , dp);
        

        dp[index][lastPicked+1] = Math.max(left , right);
